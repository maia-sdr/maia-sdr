...
class Macc(Elaboratable):
    ...
    accumulator should no carry over the additions of the products
    ...
    """
    def __init__(self, a_width, b_width, *, acc_width=48):
        self.aw = a_width
        self.bw = b_width
    ...
    def elaborate(self, platform):
        ...
        with m.If(strobe_in_q[2]):
            m.d.sync += self.acc.eq(Mux(first_acc_q[2], mult, mult + self.acc))
...
class Coefficients(Elaboratable):
    ...
    MACC. The RAM has two one port and one write port, and a read latency of 2
    cycles.
...
class FIR4DSP(Elaboratable):
    ...
    This module is a polyphase FIR decimator that uses 4 DSP48. It needs at
    least 3 clock cycles per input sample. The decimation factor is
    configurable at runtime. The maximum FIR length is 6 times the decimation
    factor, or 256, whatever is smaller, with the additional constraint that
    the FIR length must be a multiple of the decimation factor.
    ...
    def elaborate(self, platform):
        ...
        m.submodules.macc0_re = macc0_re = Macc(self.iw, self.coeff_width)
        m.submodules.macc0_im = macc0_im = Macc(self.iw, self.coeff_width)
        m.submodules.macc1_re = macc1_re = Macc(self.iw, self.coeff_width)
        m.submodules.macc1_im = macc1_im = Macc(self.iw, self.coeff_width)
        ...
        m.d.sync += [
            macc_done_q.eq(Cat(last_acc, macc_done_q[:-1])),
...
class FIR2DSP(Elaboratable):
    ...
    This module is a polyphase FIR decimator that uses 2 DSP48. It needs at
    least 3 clock cycles per input sample. The decimation factor is
    configurable at runtime. The maximum FIR length is 3 times the decimation
    factor, or 128, whatever is smaller, with the additional constraint that
    the FIR length must be a multiple of the decimation factor.
    ...
    def elaborate(self, platform):
        ...
        m.submodules.macc_re = macc_re = Macc(self.iw, self.coeff_width)
        m.submodules.macc_im = macc_im = Macc(self.iw, self.coeff_width)
        m.submodules.samples = samples = SampleBuffer(
